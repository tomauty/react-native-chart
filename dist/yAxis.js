...
label));};_this.state={bounds:{min:0,max:0}};return _this;} // Credits:  Martin Sznapka, StackOverflow, QuestionID: 9461621
_createClass(YAxis,[{key:'shortenLargeNumber',value:function shortenLargeNumber(num,useDecimal){var digits=useDecimal?2:0;var units=['K','M','B','t','P','E','Z','Y'],decimal;for(var i=units.length-1;i>=0;i--){decimal=Math.pow(1000,i+1);if(num<=-decimal||num>=decimal){return +(num/decimal).toFixed(digits)+units[i];}}return num;}},{key:'render',value:function render()
...
var unique=(0,_util.uniqueValuesInDataSet)(data[0]);
